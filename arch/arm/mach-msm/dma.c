...
void msm_dmov_stop_cmd(unsigned id, struct msm_dmov_cmd *cmd, int graceful)
{
	...
	writel_relaxed((graceful << 31), DMOV_FLUSH0(ch));//DMOV_REG(DMOV_FLUSH0(ch), adm));
	wmb();
}
...
void msm_dmov_enqueue_cmd_ext(unsigned id, struct msm_dmov_cmd *cmd)
{
	...
	status = readl_relaxed(DMOV_STATUS(ch));//DMOV_REG(DMOV_STATUS(ch), adm));
...
//			cmd->exec_func(cmd);
		...
		writel_relaxed(cmd->cmdptr, DMOV_CMD_PTR(ch));//DMOV_REG(DMOV_CMD_PTR(ch), adm));
	} else {
		if (!dmov_conf[adm].channel_active) {
...
void msm_dmov_flush(unsigned int id)
{
	...
	if (!list_empty(&dmov_conf[adm].active_commands[ch])) {
		...
		writel_relaxed(DMOV_FLUSH_TYPE, DMOV_FLUSH0(ch));//DMOV_REG(DMOV_FLUSH0(ch), adm));
	}
...
{
	errdata->flush[0] = readl_relaxed(DMOV_FLUSH0(ch));//DMOV_REG(DMOV_FLUSH0(ch), adm));
	errdata->flush[1] = readl_relaxed(DMOV_FLUSH1(ch));//DMOV_REG(DMOV_FLUSH1(ch), adm));
	errdata->flush[2] = readl_relaxed(DMOV_FLUSH2(ch));//DMOV_REG(DMOV_FLUSH2(ch), adm));
	errdata->flush[3] = readl_relaxed(DMOV_FLUSH3(ch));//DMOV_REG(DMOV_FLUSH3(ch), adm));
	errdata->flush[4] = readl_relaxed(DMOV_FLUSH4(ch));//DMOV_REG(DMOV_FLUSH4(ch), adm));
	errdata->flush[5] = readl_relaxed(DMOV_FLUSH5(ch));//DMOV_REG(DMOV_FLUSH5(ch), adm));
}
...
static irqreturn_t msm_datamover_irq_handler(int irq, void *dev_id)
{
	...
	spin_lock_irqsave(&dmov_conf[adm].lock, irq_flags);
	/* read and clear isr */
	int_status = readl_relaxed(DMOV_ISR);//DMOV_REG(DMOV_ISR, adm));
	...
	while (int_status) {
		...
		int_status &= ~mask;
		ch_status = readl_relaxed(DMOV_STATUS(ch));//DMOV_REG(DMOV_STATUS(ch), adm));
		...
		do {
			valid = 1;
			ch_result = readl_relaxed(DMOV_RSLT(ch));//DMOV_REG(DMOV_RSLT(ch), adm));
			if (list_empty(&dmov_conf[adm].active_commands[ch])) {
			...
			if (ch_result & DMOV_RSLT_ERROR) {
				...
				writel_relaxed(0, DMOV_FLUSH0(ch));//DMOV_REG(DMOV_FLUSH0(ch), adm));
			}
			rmb();
			ch_status = readl_relaxed(DMOV_STATUS(ch));//DMOV_REG(DMOV_STATUS(ch), adm));
			PRINT_FLOW("msm_datamover_irq_handler id %d, status %x\n", id, ch_status);
...
//					cmd->exec_func(cmd);
				...
				writel_relaxed(cmd->cmdptr, DMOV_CMD_PTR(ch));
