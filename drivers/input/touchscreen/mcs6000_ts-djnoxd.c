...
/*
static int melfas_init_panel(struct mcs6000_ts_device *ts)
{
	int ret ;
	int buf = 0;
...
{
	struct mcs6000_ts_device *ts = container_of(work, struct mcs6000_ts_device,work);
	int ret = 0, i;
...
{
	struct mcs6000_ts_device *dev = (struct mcs6000_ts_device *)handle;
...
#if 0 
	msleep(200);
	/* mcs6000 need STOP signal after writing addres by bongkyu.kim */
...
#else
	msleep(200); /* mcs6000 need STOP signal after writing addres by bongkyu.kim */
...
static int mcs6000_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	INIT_WORK(&dev->work, mcs6000_work);
